...
class TransLearner:
    def __init__(self, mac, logger, main_args):
        ...
        for task in self.task2args:
            ...
            self.task2encoder_params[task] = list(self.mac.task_encoder_parameters(task))
            ...
            if self.main_args.save_repre:
                ...
                self.task2train_info[task]["save_repre_t"] = -task_args.save_repre_interval - 1
    ...
    def dynamic_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str):
        ...
        for t in range(batch.max_seq_length):
            if t != batch.max_seq_length - 1:
                ...
                mi_loss =  self.mac.compute_mi_loss(batch, t=t, task=task)
        ...
        if self.main_args.save_repre and t_env - self.task2train_info[task]["save_repre_t"] >= self.main_args.save_repre_interval:
            self.mac.save_task_repres(os.path.join(self.task2repre_dir[task], f"{t_env}.npy"), task)
            self.task2train_info[task]["save_repre_t"] = t_env
        ...
        if t_env - self.task2train_info[task]["log_stats_t"] >= self.task2args[task].learner_log_interval:
            ...
            self.logger.log_stat(f"{task}/KL_loss", tot_mi_loss.item(), t_env)
