...
class TransLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # task encoder
        self.to_do_dynamic_learning = getattr(args, "pretrain", False) # True
        self.task_encoder_params = list(self.mac.task_encoder_parameters())
    ...
    def dynamic_train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        obs_pred, state_pred, reward_pred, tot_mi_loss = [], [], [], []
        for t in range(batch.max_seq_length):
            if t != batch.max_seq_length - 1:
                # mi_loss: [batch_size, n_agents]
                mi_loss =  self.mac.compute_mi_loss(batch, t=t)
                tot_mi_loss.append(mi_loss)
        ...
        tot_mi_loss = th.stack(tot_mi_loss, dim=1)
        # get target labels
        ...
        tot_mi_loss = (tot_mi_loss * mask).mean(dim=-1).sum() / mask.sum()

        task_repre_loss = pred_obs_loss + pred_state_loss + 10 * pred_reward_loss + 10 * tot_mi_loss
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("mi_loss", tot_mi_loss.item(), t_env)
    ...
    def rl_train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Mix
        task_repre_mu, _ = self.mac.get_task_repres(require_grad=False)
        bs, seq_len = chosen_action_qvals.size(0), chosen_action_qvals.size(1)
        task_repre_mu = task_repre_mu[None, None, ...].repeat(bs, seq_len, 1, 1).to(chosen_action_qvals.device)
        if self.mixer is not None:
            chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1], task_repre_mu)
            target_max_qvals = self.target_mixer(target_max_qvals, batch["state"][:, 1:], task_repre_mu)
