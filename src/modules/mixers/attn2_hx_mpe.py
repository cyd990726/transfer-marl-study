...
class QMixer(nn.Module):
    def __init__(self, decomposer, args):
        ...
        self.n_enemies, state_nf_al, state_nf_en = \
            decomposer.n_landmarks, decomposer.state_nf_al, decomposer.state_nf_en
        self.n_entities = self.n_agents + self.n_enemies

        # get action dimension information
        self.n_actions = decomposer.n_actions

        # don't state last action
        ...
        mixing_input_dim = self.entity_embed_dim + args.task_repre_dim
        entity_mixing_input_dim = self.entity_embed_dim + self.entity_embed_dim + args.task_repre_dim
