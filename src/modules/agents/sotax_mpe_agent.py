...
class SotaXMPEAgent(nn.Module):
    def __init__(self, input_shape_info, decomposer, args):
        super(SotaXMPEAgent, self).__init__()
        ...
        self.action_layer = nn.Linear(args.rnn_hidden_dim + self.task_repre_dim, n_actions)
    ...
    def forward(self, inputs, hidden_state, task_repre):
        ...
        # compute wo_action_q
        action_input = th.cat([h, task_repre.repeat(bs, 1)], dim=-1)
        q = self.action_layer(action_input)
