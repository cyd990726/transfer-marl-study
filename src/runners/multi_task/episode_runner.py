    ...
    def run(self, test_mode=False, pretrain_phase=False):
        ...
        while not terminated:
            ...
            if pretrain_phase:
                ...
                # TODO: bug here, should denote which task
                actions = self.mac.select_actions(self.batch, t_ep=self.t, t_env=0, task=self.task, test_mode=False)
            else:
        ...
        if not pretrain_phase:
            # Don't do recording when pretrain phase        
            ...
            if not self.args.evaluate and test_mode and len(self.test_returns) == self.args.test_nepisode:
                self._log(cur_returns, cur_stats, log_prefix)            
            elif not self.args.evaluate and self.t_env - self.log_train_stats_t >= self.args.runner_log_interval:
                self._log(cur_returns, cur_stats, log_prefix)
