...
def run(_run, _config, _log):
    ...
    if args.use_tensorboard and not args.evaluate:
        ...
        if args.env == "sc2":
            ...
            # mpe
            tb_logs_direc = os.path.join(dirname(dirname(abspath(__file__))), "results", args.env, args.env_args["scenario_name"], args.name)
        else:
    ...
    if args.env == "sc2":
        ...
        # mpe
        output_dirname = os.path.join(dirname(dirname(abspath(__file__))), "outputs", args.env, args.env_args["scenario_name"], args.name)
    else:
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        episode_batch = runner.run(test_mode=False, pretrain_phase=pretrain_phase) #跑一个episode的数据
